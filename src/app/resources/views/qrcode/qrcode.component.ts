...
export class QrCodeComponent implements OnInit {
    ...
    constructor(
        ...
        breadCrumService.pushStep(RESOURCES_CONSTANTS.LABEL.RESOURCES_QRCODE_LINK, RESOURCES_CONSTANTS.URL.RESOURCES_QRCODE, true);
    ...
    loadProducts() {
        this.resourcesService.getClientProducts(this.selectedProductType)
        .subscribe(
        data => {
    ...
    selectClientCompanyProduct(productId) {
        if(productId == 0){
            ...
            for (let i = 0; i < this.clientCompanyProducts.length; i++) {
                if( this.clientCompanyProducts[i].product.alias == productId){
                    this.selectedProduct = this.clientCompanyProducts[i].product;
                }
            }
