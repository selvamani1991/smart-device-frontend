...
@Component({
    ...
    templateUrl: 'vendor-error.component.html'
})

export class VendorErrorComponent implements OnInit {
    report: any= {};
    services= [];
    ...
    getComponents(productData, row){
        ...
        for (let i = (row -1)*5; i < row*5; i++) {
            if(i < productData.errorDataList.length){
                let component = productData.errorDataList[i];
    ...
    loadClientVendorReport() {
        if (this.validateForm()) {
            ...
            this.reportService.getClientErrorData(request,this.currentPage,this.pageSize)
            .subscribe(
            data => {
    ...
    downloadProductData(form) {
        if (this.validateForm()) {
            ...
            this.reportService.downloadErrorData(request)
            .subscribe(
            data => {
    ...
    getChart(productData, id) {
        var dataArray = [];
        var total =0;
        for (let i = 0; i < productData.errorDataList.length; i++) {
            var value = productData.errorDataList[i].count;
            var name = productData.errorDataList[i].name;
            if (name != '') {
                dataArray.push({'errorDataList': name, 'count': value});
                total+=value;
            }
        }
        if(total == 0){
            dataArray.push({ 'errorDataList': 'NA', 'count': 1 });
        }
        ...
        pieSeries.dataFields.category = 'errorDataList';
    ...
    getColumnBar() {
        ...
        for (let j = 0; j < this.productDatas.length; j++) {
            ...
            for (var i = 1; i <= 25; i++) {
                var value = parseInt(this.productDatas[j]['errorDataList' + i]);
                var name = this.getComponentData(this.productDatas[j],i );
                if (name != '') {
                    dataItem[name] = value;
                }
            }
        ...
        for (var i = 1; i <= 25; i++) {
            var name = self.getComponentData(this.productDatas[0], i);
            if (name != '') {
    ...
    getComponentData(productData,index) {
        if (this.product.productType) {
            console.log(this.productData.errorDataLists.length)
            for (let i = 0; i < this.productData.errorDataLists.length; i++) {
                let errorDataList = this.productData.errorDataLists[i];
                if (errorDataList.sequence == index) {
                    return errorDataList.name;
                }
            }
        }
        return '';
